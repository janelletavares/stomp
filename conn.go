...
func readLoop(c *Conn, reader *frame.Reader) {
	for {
		...
		log.Printf("readLoop: before putting on the read ch %d\n", len(c.readCh))
		c.readCh <- f
		log.Printf("readLoop: after putting on the read ch\n")
	}
}
...
func processLoop(c *Conn, writer *frame.Writer) {
	...
	for {
		...
		case <-readTimeoutChannel:
			...
			log.Printf("processLoop: readTimeout\n")
		...
		case <-writeTimeoutChannel:
			...
			log.Printf("processLoop: writeTimeout\n")
		...
		case f, ok := <-c.readCh:
			log.Printf("processLoop: readChannel %d\n", len(c.readCh))
			// stop the read timer
			if readTimer != nil {
				readTimer.Stop()
				readTimer = nil
		...
		case req, ok := <-c.writeCh:
			log.Printf("processLoop: writeChannel %d\n", len(c.writeCh))
...
func (c *Conn) sendFrame(f *frame.Frame) error {
	...
	log.Printf("sendFrame: waiting for Mutex\n")
	c.closeMutex.Lock()
	log.Printf("sendFrame: got Mutex\n")
	if c.closed {
	log.Printf("sendFrame: closed\n")
		c.closeMutex.Unlock()
		c.conn.Close()
